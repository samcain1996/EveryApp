...
public class GUI extends JFrame {
	...
	void viewRequests() {
		...
		Scanner reader;
		try {
			reader = new Scanner(requestsFile);
			while (reader.hasNext()) {
				requests.add(reader.nextLine());
			}
			reader.close();
		...
		DefaultListModel<String> requestsModel = new DefaultListModel<String>();
	...
	private class logInListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try {
				...
				if (logInFound) {
					user = new User(UserType.ADMIN, tBox1.getText());
					displayApps();
				} else
	...
	private class submitRequestListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				writer.println(tBox1.getText() + " " + tBox2.getText() + " " + user.getName());
				writer.close();
